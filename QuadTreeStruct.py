...
class Node:
    ...
    def subdivide(self):
        ...
        self.children.append(
            Node(matrix1, self.cell_size, self.x1, self.y1, self.x1 + len(self.matrix[0]) / 2, self.y1 + len(self.matrix) / 2))
        # Prawa gora
        self.children.append(
            Node(matrix2, self.cell_size, self.x1 + len(self.matrix[0]) / 2, self.y1, self.x2, self.y1 + len(self.matrix) / 2))
        # Lewy dol
        self.children.append(
            Node(matrix3, self.cell_size, self.x1, self.y1 + len(self.matrix) / 2, self.x1 + len(self.matrix[0]) / 2, self.y2))
        # Prawy dol
        self.children.append(
            Node(matrix4, self.cell_size, self.x1 + len(self.matrix[0]) / 2, self.y1 + len(self.matrix) / 2, self.x2, self.y2))
    ...
    def build_tree_recursive(self):
        ...
        if objects > 4:
            ...
            if self.matrix.shape[0] > 8 * self.cell_size and self.matrix.shape[1] > 8 * self.cell_size:
                self.subdivide()
    ...
    def print_tree_recursive(self, image):
        ...
        for child in self.children:
            if child.is_not_leaf():
                ...
                if child.check_for_elements() >= 0.5 * len(child.matrix) * len(child.matrix[0]):
        ...
        for child in self.children:
            if child.is_not_leaf():
                ...
                if child.check_for_elements() >= 0.5 * len(child.matrix) * len(child.matrix[0]):
                    ...
                    p3 = (child.x2, child.y1)
                    p4 = (child.x1, child.y2)
                    t1 = (points.index(p1), points.index(p2), points.index(p3))
                    t2 = (points.index(p1), points.index(p2), points.index(p4))
    ...
    def save_to_txt(self, filename):
        ...
        with open(filename, 'w') as file:
            ...
            for i, (node1, node2, node3) in enumerate(triangles):
                file.write(f"{i} {node1} {node2} {node3}\n")
...
class QuadTreeStruct:
    ...
    def save_to_txt(self):
        self.root.save_to_txt("StructuralMesh.txt")
